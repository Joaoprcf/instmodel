...
def test_simple_dense_model():
    ...
    Then validates that the exported instruction model matches the PyTorch outputs.
    """
    ...
    torch_pred = model_graph.predict(x_data, verbose=0)
    ...
    validate_torch_model(model_graph, instruction_model["validation_data"])

    print(
        "Simple Dense model validation successful: Instruction model matches PyTorch output."
    )
...
def test_complex_attention_model():
    """
    Tests a complex attention-based model and validates both the instruction model and PyTorch model.
    """
    ...
    validate_torch_model(model_graph, result["validation_data"])
...
def test_nested_model():
    ...
    validate_instruction_model(result)
    validate_torch_model(final_model, result["validation_data"])
...
def test_embeddings(capsys):
    ...
    choices = [
        x_data[:, 0] + x_data[:, 2],
        x_data[:, 0] - x_data[:, 2] * 0.5,
    ]
    default_choice = -x_data[:, 0] + x_data[:, 2] * 1.5
    ...
    *_, inst_output_y = instruction_model_inference(instmodel, x_data)
    ...
    assert np.allclose(inst_output_y, pred_after, atol=1e-6), (
        "Instruction model output does not match PyTorch output after training."
    )
...
def test_scale_and_shift_vectorized():
    ...
    torch_pred = model.predict(x_data, verbose=0)

    expected = x_data * np.array([2.0, 0.5, 3.0]) + np.array([1.0, -1.0, 0.0])
    assert np.allclose(torch_pred, expected, atol=1e-6)
...
def test_reduce_sum():
    ...
    torch_pred = model.predict_on_batch(x_data)
    assert np.allclose(torch_pred, expected, atol=1e-6)
...
def test_gelu_activation():
    """
    Tests the GeLU activation function matching PyTorch's default (exact erf-based).
    ...
    torch_gelu = torch.nn.functional.gelu(torch.tensor(test_input)).numpy()

    numpy_gelu = test_input * 0.5 * (1 + erf(test_input / np.sqrt(2)))

    assert np.allclose(torch_gelu, numpy_gelu, atol=1e-6), (
        f"NumPy GeLU does not match PyTorch GeLU. "
        f"PyTorch: {torch_gelu}, NumPy: {numpy_gelu}"
    )
    ...
    torch_pred2 = model2.predict(x_data2, verbose=0)

    inst_model2 = model2.create_instruction_model()
    *_, inst_output = instruction_model_inference(inst_model2, x_data2)

    assert np.allclose(inst_output, torch_pred2, atol=1e-6), (
        "Instruction model GeLU output does not match PyTorch output."
    )
...
def test_softplus_activation():
    ...
    torch_softplus = torch.nn.functional.softplus(torch.tensor(test_input)).numpy()

    numpy_softplus = np.maximum(test_input, 0) + np.log1p(np.exp(-np.abs(test_input)))

    assert np.allclose(torch_softplus, numpy_softplus, atol=1e-6), (
        f"NumPy softplus does not match PyTorch softplus. "
        f"PyTorch: {torch_softplus}, NumPy: {numpy_softplus}"
    )
    ...
    *_, inst_output = instruction_model_inference(inst_model2, x_data2)
    ...
    assert np.allclose(inst_output, torch_pred2, atol=1e-6), (
        "Instruction model softplus output does not match PyTorch output."
    )
...
def test_multiply_heads():
    ...
    attention = np.array([[2.0, 3.0, 4.0, 5.0]])

    torch_pred = model.predict([data, attention], verbose=0)
...
def test_multiply_heads_scalar():
    ...
    torch_pred = model.predict([data, scalar], verbose=0)
    expected = data * 2.0
    assert np.allclose(torch_pred, expected, atol=1e-6)
